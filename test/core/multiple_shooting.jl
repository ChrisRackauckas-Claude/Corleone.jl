...
@testset "General Multiple shooting tests" begin
    ...
    @test isapprox(
        Corleone.shooting_constraints(traj), [
            1.375754960969482,
            0.22357357513551113,
            1.2417260108009396,
            1.3757549609694817,
    ...
    res = zeros(30)
    @test isapprox(
        Corleone.shooting_constraints!(res[10:24], traj), [
            1.375754960969482,
            0.22357357513551113,
            1.2417260108009396,
            1.3757549609694817,
...
@testset "Initialization" begin
    ...
    @testset "Linear" begin
        ...
        @test isapprox(ps.interval_2.u0, u0 .+ ([2.0, 1.0, 1.34] .- u0) * 3 / 12, atol = 1.0e-7)
        @test isapprox(ps.interval_3.u0, u0 .+ ([2.0, 1.0, 1.34] .- u0) * 6 / 12, atol = 1.0e-7)
        @test isapprox(ps.interval_4.u0, u0 .+ ([2.0, 1.0, 1.34] .- u0) * 9 / 12, atol = 1.0e-7)
    end
    ...
    @testset "Hybrid" begin
        ...
        @test isapprox(ps.interval_3.u0, [0.2875960819428889, 0.27699894224421623, -1.088467272254529], atol = 1.0e-10)
